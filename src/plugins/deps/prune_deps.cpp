...
namespace nanos {
   namespace ext {
      ...
      class PruneDependenciesDomain : public BaseDependenciesDomain
      {
         private:
         ...
         public:
            PruneDependenciesDomain() : BaseDependenciesDomain(), _addressDependencyMap() {}
            PruneDependenciesDomain ( const PruneDependenciesDomain &depDomain )
               : BaseDependenciesDomain( depDomain ),
               _addressDependencyMap ( depDomain._addressDependencyMap ) {}
            
            ~PruneDependenciesDomain()
            {
      ...
      template void PruneDependenciesDomain::submitDependableObjectInternal ( DependableObject &depObj, DataAccess* begin, DataAccess* end, SchedulePolicySuccessorFunctor* callback );
      template void PruneDependenciesDomain::submitDependableObjectInternal ( DependableObject &depObj, std::vector<DataAccess>::iterator begin, std::vector<DataAccess>::iterator end, SchedulePolicySuccessorFunctor* callback );
      
      /*! \brief Default plugin implementation.
       */
      class PruneDependenciesManager : public DependenciesManager
      {
         public:
            PruneDependenciesManager() : DependenciesManager("Nanos prune dependencies domain") {}
            virtual ~PruneDependenciesManager () {}
            
            /*! \brief Creates a default dependencies domain.
             */
            DependenciesDomain* createDependenciesDomain () const
            {
               return NEW PruneDependenciesDomain();
            }
      };
  
      class PruneDepsPlugin : public Plugin
      {
            
         public:
            PruneDepsPlugin() : Plugin( "Nanos++ prune dependencies management plugin",1 )
            {
            }
            ...
            virtual void init()
            {
               sys.setDependenciesManager(NEW PruneDependenciesManager());
            }
      };
...
DECLARE_PLUGIN("deps-prune",nanos::ext::PruneDepsPlugin);
